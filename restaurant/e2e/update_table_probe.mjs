...
console.log('DB before', JSON.stringify(await p.evaluate(async (t) => (await frappe.call('frappe.client.get_value', { doctype: 'Restaurant Object', filters: { name: t }, fieldname: ['no_of_seats', 'description'] })).message, TABLE)));
await d.locator('input[data-fieldname="description"]').fill(TABLE);   // unchanged name: no rename, just a save
await d.getByRole('button', { name: /^Save$/ }).click({ force: true }); await p.waitForTimeout(4000);
console.log('DB after', JSON.stringify(await p.evaluate(async (t) => (await frappe.call('frappe.client.get_value', { doctype: 'Restaurant Object', filters: { name: t }, fieldname: ['no_of_seats', 'description'] })).message, TABLE)));
await b.close();
